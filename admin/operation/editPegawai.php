	...
	if (!isset($_POST["submit"])){
        ...
        //update data into database  
	  	if ($valid2 && $valid3 && $valid4 && $valid5 && $valid6 && $valid7){   //escape inputs data   
            ...
            $query = " UPDATE pegawai SET nama_lengkap='".$name."', email='".$email."', username='".$username."', password='".$password."', level='".$level."' WHERE idpegawai=".$id." ";   // Execute the query   
            $result = $db->query( $query );   
            if (!$result){      
                die ("Could not query the database: <br />". $db->error);   
            }else{
		...
		<form method="POST" autocomplete="on" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]).'?id='.$id;?>">
			<table>
                <tr>
                    ...
                    <td valign="top">:</td>
                    <td valign="top"><input type="text" name="idpegawai" size="30" maxlength="10" placeholder="ID Pegawai" autofocus value="<?php echo $id;?>" disabled></td>
